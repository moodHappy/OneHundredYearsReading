    ...
    <script>
        ...
        async function pushToGitHub() {
            ...
            try {
                ...
                if(res.ok) {
                    alert("🎉 矩阵记录成功！云端流水线已启动，日历将在稍后自动刷新！");
        ...
        // 终极防崩溃：采用隐藏的 JSON Base64 数据桥接生成独立页面
        function generateTemplate(title, orig, anal) {
            const dataObj = { title: title, orig: orig, anal: anal };
            const b64Data = btoa(unescape(encodeURIComponent(JSON.stringify(dataObj))));
...
<head>
    ...
    <style>
        ...
        .card-header { font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; color: #95a5a6; border-bottom: 1px solid var(--border); padding-bottom: 10px; margin-bottom: 15px; font-weight: bold;}
        ...
        .edit-textarea { width: 100%; min-height: 250px; padding: 15px; font-family: monospace; font-size: 15px; border: 2px dashed var(--accent); border-radius: 10px; box-sizing: border-box; resize: vertical; margin-bottom: 15px; background: #fafafa; }
...
<body>
    <div class="container">
        ...
        <script id="page-data" type="application/base64">${b64Data}</scr` + `ipt>

        <div class="card" id="block-original">
            <div class="card-header">📖 原始文本 (Original)</div>
            <div id="view-original" class="view-mode markdown-body" style="white-space: pre-wrap; font-family: Georgia, serif; font-size: 1.15rem; line-height: 1.8; color: #34495e;"></div>
        </div>
        ...
        <div class="card" id="block-analysis">
            ...
            <div id="edit-analysis" class="edit-mode">
                ...
                <div style="display:flex;">
                    <button class="save-btn" onclick="saveEdit()">💾 保存并更新云端</button>
    ...
    <script>
        // 解析底层数据
        let pageData = JSON.parse(decodeURIComponent(escape(atob(document.getElementById('page-data').textContent))));

        function renderAll() {
            document.getElementById('view-original').textContent = pageData.orig;
            if (typeof marked !== 'undefined') {
                document.getElementById('view-analysis').innerHTML = marked.parse(pageData.anal);
            } else {
                document.getElementById('view-analysis').innerHTML = "<pre style='white-space:pre-wrap;'>" + pageData.anal + "</pre>";
            }
        }
        ...
        const textarea = document.getElementById('textarea-analysis');
        ...
        function triggerEdit() {
            viewBlock.classList.add('editing');
            editBlock.classList.add('active');
            textarea.value = pageData.anal;
        }

        function cancelEdit() {
            viewBlock.classList.remove('editing');
        ...
        async function saveEdit() {
            ...
            // 提取新内容并更新底层保险箱
            pageData.anal = textarea.value;
            document.getElementById('page-data').textContent = btoa(unescape(encodeURIComponent(JSON.stringify(pageData))));
            ...
            try {
                ...
                const putRes = await fetch(\`https://api.github.com/repos/\${owner}/\${repo}/contents/\${fileRelPath}\`, {
                    ...
                    headers: { 'Authorization': \`token \${token}\`, 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        message: \`Update analysis via UI\`,
