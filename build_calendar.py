...
<body>
    ...
    <div class="modal" id="modal">
        <div class="modal-content">
            ...
            <p style="font-size:0.8rem; color:#888; margin-top:-10px;">仅需 Token，仓库已自动锁定！</p>
    ...
    <script>
        ...
        async function pushToGitHub() {
            ...
            try {
                // btoa 需要将 Unicode 转换为 Latin1 以免报错
                ...
                if(res.ok) {
                    alert("🎉 记录成功！日历将在几秒后通过 Action 自动刷新。");
        ...
        // 终极防崩溃：利用原生 textarea 保存原始数据，极度安全，解析零失误！
...
<body>
    ...
    <script>
        ...
        function renderAll() {
            // 原文保留换行，分析使用 marked.js 渲染 Markdown
        ...
        async function saveEdit() {
            ...
            document.getElementById('raw-analysis').innerHTML = encodeHTML(newText);
            ...
            try {
                // btoa 乱码终极解决方案：使用 TextEncoder
                const utf8Encode = new TextEncoder().encode(newHtml);
                const binaryString = String.fromCodePoint(...utf8Encode);
                const base64Html = btoa(binaryString);
